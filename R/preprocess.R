...
#' @description Select the motif related to the input genes. Genes used here should be ENSEMBEL ID.
...
#' @param gene character, indicating genes used to filter motif. You can use differentially expressed genes of corresponding scRNA-seq or bulk RNA-seq data, or just use expressed genes.
#'
...
#' @param count_all data.frame, indicating counts of peaks, generated by htseq-count. If you calculate counts for each sample separately, you need to combine these counts firstly through cbind().
#' @param gtf data.frame, indicating gtf file of all peaks, generated by \link{generate_gtf} function
#'
...
#' @description Identify differential peaks based on edgeR, samples of peaks need to be divided into at least two groups.
#' @param Count data.frame, indicating counts of peaks, generated by htseq-count. If you calculate counts for each sample separately, you need to run merge_sort_count() function firstly.
...
generate_gtf <- function(peak){
  ...
  for (i in 1:nrow(Exp1)) {
    if (str1 == Exp1[i,1] & (start1 >= Exp1[i,2]&start1<=Exp1[i,3] | end1>=Exp1[i,2]&end1<=Exp1[i,3] | start1<=Exp1[i,2]&end1>=Exp1[i,3] | Exp1[i,2]>end1&(Exp1[i,3]-Exp1[i,2]+end1-start1+Exp1[i,2]-end1)<width1*2 | start1>Exp1[i,3]&(Exp1[i,3]-Exp1[i,2]+end1-start1+start1-Exp1[i,3])<width1*2)) {
      ...
      if (start12 < 1) {
        start12 = 1
        end12 = start12 + width*2
      }
      out1 <- paste0(str1,'\t','.','\t','exon','\t',start12,'\t',end12,'\t','.','\t','.','\t','.','\t','gene_id','\t',paste0('Peak',no1))
      col1 <- c(col1,out1)
      no1 = no1 + 1
      str1 = Exp1[i,1]
      start1 = Exp1[i,2]
