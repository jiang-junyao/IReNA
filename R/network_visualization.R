...
#' @param enrichment Enrichment analysis results of genes in each module, generated by \code{\link{enrich_module}}. If it is not NULL, when plotting the Intramodular Network, the functions enriched by each module will be plotted in the diagram
...
#' @param edge.color edge color. You need to input two colors, first one indicate 'Positive' regulation, second one indicate 'Negative' regulation.
...
plot_network <- function(TFs_list, enrichment = NULL,layout, group.cols = NULL, title.name = NULL, vertex.size = 13, type = "TF", vertex.label.color = 'black', edge.label.color = 'black'
                         ,legend = TRUE, vertex.label.cex = 0.6, vertex.label.family = 'ArialMT', frame.color = 'white'
                         , arrow.size = 0.2, arrow.width = 0.5, edge.with = 1.8, edge.curved = 0, edge.color = c('#FDD1B0','#B3B3B3'),
...
#' @param vertex.size.add expansion size of vertex whose edge numbers are last 25%-75% and top25%
#' @param vertex.label.color ertex label color
#' @param edge.label.color edge label color
#' @param legend logic, indicating whether to show the legend
#' @param vertex.label.cex The label size of vertex
...
#' cor0.6 <- get_cor(Kmeans_clustering, Tranfac201803_Hs_MotifTFsF, 0.7, start_column=3)
#' TFs_list <- network_analysis(cor0.6,Kmeans_clustering)
#' plot_network(TFs_list)
plot_tf_network <- function(TFs_list, layout = 'grid', group.cols = NULL, title.name = NULL, vertex.size = 13, vertex.size.add = 3,vertex.label.color = 'black', edge.label.color = 'black'
                            ,legend = TRUE, vertex.label.cex = 0.8, vertex.label.family = 'ArialMT', frame.color = 'white'
                            , arrow.size = 0.2, arrow.width = 0.5, edge.width = 1.8, edge.curved = 0, edge.color = c('#FDD1B0','#B3B3B3')) {
  if (is.null(group.cols)) {
    col1 <- c('#67C7C1','#5BA6DA','#FFBF0F','#C067A9','#EF9951','#E56145','#C0C130','#67C1E3','#EF9951','#00BFC4','#AEC7E8','#E56145','#2F4F4F')
  } else {
    col1 <- group.cols
  }
  network <- TFs_list[["TF_network"]]
  ...
  colnames(edges) <- c("from", "to", "type", "weight")
  g <- igraph::graph_from_data_frame(edges, vertices = nodes, directed = TRUE)
  tf_degree <- as.data.frame(table(tf_network$TFSymbol))
  cut1 <- as.numeric(summary(as.data.frame(table(tf_network$TFSymbol))[,2])[2])
  cut2 <- as.numeric(summary(as.data.frame(table(tf_network$TFSymbol))[,2])[3])
  cut3 <- as.numeric(summary(as.data.frame(table(tf_network$TFSymbol))[,2])[5])
  vertex.size1 <- c()
  for (i in 1:nrow(nodes)){
    if(nodes[i,1] %in% tf_degree[,1]){
      ...
      else if(tf_degree[tf_degree[,1]==nodes[i,1],][,2] < cut3 & tf_degree[tf_degree[,1]==nodes[i,1],][,2] > cut1) {
  ...
  if (legend == TRUE) {
    ...
    legend(x = 1.3, y = 1.3, paste0('Module',levels(factor(igraph::V(g)$type))), pch = 21, col = "#777777", pt.bg = col1)
...
#' TFs_list <- network_analysis(cor0.6,Kmeans_clustering)
#' #enrichment <- enrich_module(Kmeans_cluster_Ens, org.Hs.eg.db, 'KEGG')
#' #plot_network(TFs_list,enrichment,layout = 'random')
plot_intramodular_network <- function(TFs_list, enrichment = NULL,layout = 'circle', group.cols = NULL, title.name = NULL, vertex.size = 25, vertex.label.color = 'black', edge.label.color = 'black'
                                      ,legend = TRUE, vertex.label.cex = 1.1, vertex.label.family = 'ArialMT', frame.color = 'white'
                                      , arrow.size = 0.5, arrow.width = 0.8, edge.width = 2.5, edge.curved = 0, edge.color = c('#FDD1B0','#B3B3B3'),
                                      vertex.label.degree = 0.35, vertex.label.dist = -6) {
  if (is.null(group.cols)) {
    col1 <- c('#67C7C1','#5BA6DA','#FFBF0F','#C067A9','#EF9951','#E56145','#C0C130','#67C1E3','#EF9951','#00BFC4','#AEC7E8','#E56145','#2F4F4F')
  } else {
