...
find_motifs <- function(motif, step = 20, fimodir,outputdir1,outputdir, Motifdir, sequencedir) {
  con1 <- motif
  no1 <- 1
  out2 <- c()
  for (i in seq(1, nrow(con1), by = step)) {
    ...
    if (nrow(con1) - i > step) {
      ...
      write.table(out1, paste0(outputdir1, "/Fimo", no1, ".sh"), row.names = FALSE, col.names = FALSE, quote = FALSE)
    } else {
  ...
  write.table(out2, paste0(outputdir1, "/Fimo", "_All", ".sh"), row.names = FALSE, col.names = FALSE, quote = FALSE)
}
