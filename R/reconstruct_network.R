...
#' @description Refine regulatory relationships through RcisTarget. Before run this function, you should download Gene-motif rankings database from https://resources.aertslab.org/cistarget, and set set the Rankingspath1 as the path of downloaded Gene-motif rankings database.
...
#' @description This function calculates the significant transcription factors and significant regulatory relationships in each module through hypergeometric tests to construct regulatory networks
#' @param regulatory_relationships Refined or unrefined regulatory relationships. Unrefined regulatory relationships are generated by \code{\link{get_cor}}, and you can use \code{\link{filter_regulation} or consequence of \code{\link{Footprints_FOS} to refine it
#' @param Kmeans_result Kmeans result data.frame, row names should be ENSEMBEL ID, and the first column should be gene Symbol ID, the second column should be KmeansGroup
#' @param TFFDR1 numeric, indicating the cutoff (FDR) to identify enriched transcription factors
#' @param TFFDR2 numeric, indicating the cutoff (FDR) to identify enriched transcription factors that regulate each module to make regulatory network for enriched transcription factors of each module
#' @param ModuleFDR numeric, indicating the cutoff (FDR) to identify enriched modular regulatory relationships to make intramodular regulatory network
...
#' @export
#'
#' @examples Kmeans_cluster_Ens <- add_ENSID(clustering,Spec1='Hs')
#' motif1 <- Tranfac201803_Hs_MotifTFsF
#' regulatory_relationships <- get_cor(Kmeans_cluster_Ens,motif1,0.6)
#' TFs_list=network_analysis(regulatory_relationships,Kmeans_cluster_Ens)
network_analysis <- function(regulatory_relationships, Kmeans_result, TFFDR1 = 2, TFFDR2 =10, ModuleFDR = 0.05){
  TFs_list <- get_Enriched_TFs(regulatory_relationships, Kmeans_result, TFFdrThr1 = TFFDR1)
...
#' @description Calculate enriched transcription factors in each module through hypergeometric test
...
#' @description This function integrate clusterprofile make enrichment analysis for each moudle. Before run this function, you need to download the org.db for your species through BiocManager.
#' @param Kmeans_result Kmeans result data.frame, row names should be ENSEMBEL ID, and the first column should be gene Symbol ID, the second column should be KmeansGroup
...
#' }
enrich_module <- function(Kmeans_result, org.db, enrich.db, fun_num = 5, pvalueCutoff = 0.05) {
