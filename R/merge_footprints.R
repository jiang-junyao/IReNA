...
#' @description This function first merges footprints whose distances is less than 4, and then obtains the sequence of each footprint based on the reference genome, and  output the sequence in FASTA format
#'
#' @param footprints footprints file, generated by dnase2tf or Hint or other related software, name of first column should be 'chr', name of second column should be 'start' and name of thrid column should be 'end'.
...
getfasta <- function(merged_footprints, fastadir) {
  ...
  for (i in 1:nrow(merged_footprints)) {
    ...
    if (merged_footprints[i, 3] > length(fasta[[merged_footprints[i, 1]]])) {
      sequence <- toupper(as.character(fasta[[merged_footprints[i, 1]]][(merged_footprints[i, 2] + 1):length(fasta[[merged_footprints[i, 1]]])]))
      name <- paste0(">", merged_footprints[i, 1], ":", merged_footprints[i, 2], "-", length(fasta[[merged_footprints[i, 1]]]))
    } else{
